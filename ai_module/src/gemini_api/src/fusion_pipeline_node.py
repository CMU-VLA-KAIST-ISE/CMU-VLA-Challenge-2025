...
"""
Fusion (One-Shot) Pipeline — prompt-triggered, single Gemini call + dummyVLM waypoint publisher
...
class OneShotFusionNode:
    def __init__(self):
        ...
        self.publish_goal_on_single = rospy.get_param("~publish_goal_on_single", False)

        # dummyVLM waypoint 방식(추천)
        self.use_waypoint_interface    = rospy.get_param("~use_waypoint_interface", True)
        ...
        self.sg_enable  = rospy.get_param("~sg_enable", True)
        ...
        rospy.Subscriber("/camera/image", Image, self.cb_image, queue_size=1)
        rospy.Subscriber("/color_scan_rel",       PointCloud2, self.cb_cloud, queue_size=1)
        rospy.Subscriber("/color_scan_relative",  PointCloud2, self.cb_cloud, queue_size=1)
    ...
    def _run_once_pipeline(self, prompt: str):
        ...
        try:
            ...
            try:
                if self.use_waypoint_interface and len(centroids_map) >= 1:
