...
def scan2pixels_mecanum(laserCloud: np.ndarray, CAM: dict, LIDAR: dict) -> np.ndarray:
    ...
    rospy.loginfo(f"cam offset : {cam_offset}")
...
{self.challenge_question}"""
        ...
        if prompt is None:
            _prompt = (f"Segment Only {ob_list} in the image. Do not label as 'unknown'")
        else:
            _prompt = prompt
...
class OneShotFusionNode:
    def __init__(self):
        ...
        self.pub_scene_graph_image = rospy.Publisher("/image_scenegraph",Image,queue_size=1)
        ...
        rospy.Subscriber("/publish_scene_graph", Bool, self.cb_generate_sg, queue_size=1)
        self.pub_sg = False
    ...
    def cb_generate_sg(self, msg: Bool):
        self.pub_sg = msg.data
    ...
    # -------------------- 세그 타임아웃 래퍼
    def _attempt_once(self, image, prompt, timeout_sec):
        q = queue.Queue(maxsize=1)

        def _target():
            try:
                res = self.segmenter.run_once(image, prompt)
                q.put(("ok", res))
            except Exception as e:
                q.put(("err", (type(e).__name__, repr(e))))
        ...
        if t.is_alive():
            raise cf.TimeoutError(f"run_once timed out after {timeout_sec}s")

        status, payload = q.get_nowait()
        if status == "ok":
            return payload
        else:
            err_type, err_repr = payload
            raise RuntimeError(f"{err_type}: {err_repr}")

    def _worker(self, worker_id, image, prompt, timeout_sec, max_attempt, stop_event):
        for idx in range(max_attempt):
            if stop_event.is_set():
                return None
            try:
                result = self._attempt_once(image, prompt, timeout_sec)
            except Exception as _e_worker:
                print(f"[{now_ts_str()}] worker {worker_id}, {idx}/{max_attempt} | {repr(_e_worker)}")
                continue
            else:
                stop_event.set()
                print(f"[{now_ts_str()}] worker {worker_id}, {idx}/{max_attempt} | Success")
                return result
        return None

    def _seg_with_timeout(self, image, prompt, timeout_sec: float, num_workers: int = 4, max_attempt: int = 4):
        image = copy.deepcopy(image)
        stop_event = threading.Event()

        with cf.ThreadPoolExecutor(max_workers=num_workers) as thread_pool:
            worker_futures = [thread_pool.submit(self._worker, i, image, prompt, timeout_sec, max_attempt, stop_event) for i in range(num_workers)]

            for fut in cf.as_completed(worker_futures):
                try:
                    val = fut.result()
                except Exception:
                    val = None
            ...
            print(f"[{now_ts_str()}] Use backup segmenter")
            items, raw = self.backup_segmenter.run_once(image, prompt)
            return items, raw
            ...
            if dwell >= self.stop_dwell_sec:
                if self.pub_sg:
                    ...
                    if (now - self._auto_last_fire) >= self.auto_cooldown_sec:
                        ...
                        with self.lock:
                            if (not self.is_busy) and (self.last_image is not None) and (
                                    self.last_cloud_np is not None):
                                self.is_busy = True
                                self.push_items()
    ...
    def _run_once_pipeline(self, prompt: str):
        ...
        try:
            ...
            rospy.loginfo(f"cloud stamp: {self.popped_item['cloud_stamp']}, image stamp: {self.popped_item['image_stamp']}")
            ...
            try:
                ...
                items, raw_text = self._seg_with_timeout(
            ...
            # Compute average per-object rel_score and save image with the average score
            avg_rel_score = None
            try:
                scores = []
                for it in items:
                    try:
                        s = getattr(it, "rel_score", None)
                        if s is not None:
                            s = int(s)
                            s = max(0, min(100, s))
                            scores.append(s)
                        else:
                            print(f'{getattr(it, "label", "unknown")} no rel_score')
                    except Exception:
                        print(f'{getattr(it, "label", "unknown")} no rel_score error')
                        pass
                if scores:
                    avg_rel_score = int(round(sum(scores) / float(len(scores))))
                    try:
                        rel_dir = "/tmp/relativeness"
                        os.makedirs(rel_dir, exist_ok=True)
                        fname = f"image_relativeness_{run_idx}_{avg_rel_score}.png"
                        self.popped_item["image_pil"].save(os.path.join(rel_dir, fname), "PNG")
                        self.popped_item["image_pil"].save(os.path.join(out_root, fname), "PNG")
                        rospy.loginfo(f"[fusion] saved relativeness image: avg_score={avg_rel_score}")
                    except Exception as e:
                        rospy.logwarn(f"[fusion] failed to save relativeness image: {e}")
            except Exception as e:
                rospy.logwarn(f"[fusion] computing avg relativeness score failed: {e}")
            if not items:
            ...
            self._write_summary(out_root, "ok", labels, picks_per_seg, centroids_rel, centroids_map, frame_src, relativeness_score=avg_rel_score)
            status_payload = {"ok": True, "n": len(centroids_map), "labels": labels, "segments": segments_out}
            if avg_rel_score is not None:
                status_payload["relativeness_score"] = int(avg_rel_score)
    ...
    def _save_seg_artifacts(self, seg_dir: str, items: List[SegItem], raw_text: Optional[str]):
        ...
        for it in items:
            ...
            x1b = int(b[2]/1000.0*W); y1b = int(b[3]/1000.0*H)
            out.append({
                "label": _safe_label(getattr(it, "label", "unknown")),
                "box_2d": b,
                "box_px_tryA": [x0a,y0a,x1a,y1a],
                "box_px_tryB": [x0b,y0b,x1b,y1b],
                "mask_len": len(getattr(it, "mask", "") or ""),
                "rel_score": (int(getattr(it, "rel_score")) if getattr(it, "rel_score", None) is not None else None)
            })
    ...
    def _write_summary(self, out_root: str, note: str, labels: List[str],
                       ...
                       relativeness_score: Optional[int]=None):
        data = {
            "note": note,
        ...
        if relativeness_score is not None:
            data["relativeness_score"] = int(relativeness_score)
        os.makedirs(out_root, exist_ok=True)
